...
| 生产机沙箱 | 1457 通过 · **3 跳过** —— 有宿主 Hermes 源码，无 git 工作区（沙箱是白名单拷贝，不含 `.git`）。**这一行是按轴推导**：1460 减去**沙箱缺席的两条轴** —— git 工作区（1 条）与 `ruff` 未安装（2 条，生产 venv 不装 lint 工具）共 3 条；最后一次真实沙箱实测是 **859 通过 · 1 跳过**，跑在 v20.0 提交树上（当时总数 860） |
...
> 1448 是本机实测；1457 是**按轴推导**（1460 减去沙箱缺席的两条轴：git 工作区 1 条 + `ruff` 未安装 2 条）—— 最后一次真实沙箱实测是 859，
