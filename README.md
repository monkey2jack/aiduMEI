...
| 全轴齐备 | 1254 全绿 · 0 跳过 —— **2026-08-26 生产机候选树实测**（bundle 克隆含 `.git`，十条轴同时齐备）。上一次全轴实测是 1232 全绿（同日早前的 v20.1 候选树）。更早这一行是推导值，标注为「从未实测」长达数个版本 |
...
> `有宿主：1254 passed` 要十条轴同时齐备才拿得到 —— 2026-08-26 在生产机候选树上**实测到**
> （总数 1254，十轴齐备、0 跳过）。但别把「装上宿主」当成「全绿」：宿主只是十条轴里的一条。
