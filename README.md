...
> **当前公开正式版 v20.4 —— 优忆思：一行 Prompt 全自动部署 · 双引擎自动挡 · 市面独一份。**
...
| 维度 | 现状 |
|------|------|
| 用例总数 | **1835**（`pytest --collect-only` 实测，2026-09-09，v20.4.0 本树） |
| 独立开发机 | 1823 通过 · **12 跳过** —— **2026-09-09 实测**（v20.4.0 本树，Python 3.12；完整 extras + 模型缓存，只缺 Hermes 宿主） |
| 基础安装路径 | 1777 通过 · **35 跳过** —— 只装 `requirements.txt` + `requirements-dev.txt`（**2026-09-08 干净 venv 实测**，v20.4.0-alpha 候选树，Python 3.12） |
| 生产机沙箱 | **待复测**（按轴推导值为 1827/9，上一实测基线 2026-09-08 alpha 树 1803+9；换树后须生产机实机重测） |
| 全轴齐备 | 1834 通过 · **1 跳过** —— **2026-09-09 生产机实测**（v20.4.0 本树，独立全轴 venv：工具、extras、宿主源码、模型缓存与公开 LoCoMo 数据集齐备；那 1 跳过为本树新增用例的条件轴） |
...
| # | 环境 | 通过 | 跳过 | 跳过归因 |
|---|------|-----:|-----:|---------|
| ① | 独立开发机 · 完整 extras + 模型缓存 | 1823 | 12 | 2026-09-09 实测（v20.4.0 本树；只缺宿主 Hermes ×12） |
...
| ⑤ | 全轴齐备 · 有 `.git` · 工具 · extras · 宿主 · 模型缓存 · 数据集 | 1834 | 1 | 2026-09-09 实测（v20.4.0 本树，生产机独立全轴 venv）：1 条本树新增条件轴 |

每行 `通过 + 跳过` 对应各自测试树的总数：① 开发机为本树（1835 条）2026-09-09 实测；④⑤ 生产机沙箱/全轴为 2026-09-08 alpha 树（1812 条）实测，本树（1835）待生产机实机复测；②③ 为 1499 条历史基线，已被取代。历史对照的用途和测量规则见 [docs/TESTING.md](docs/TESTING.md#历史矩阵的口径)，公开候选未复测的环境不能沿用旧结果冒充新测量。

> **⚠️ 这些数字对应「装齐可选依赖」的环境**（v20.2.5 补记，外审指出的口径缺口）。
>
> 2026-09-09 完整开发环境实测为 1835/1823/12，前提是：`regex`、`nltk`、`numpy`、`qdrant_client`、
...
> **为什么要把 1823 和 1803 都写出来**：1823 是本树开发环境 2026-09-09 实测（缺宿主 ×12）；1803 是生产机独立沙箱的上一实测基线（2026-09-08 alpha 树，缺 `ruff` ×3、`mcp` ×5、本地嵌入模型缓存 ×1，本树待复测）——两者的跳过轴不同，数字必须与环境、日期和测试树一起读。
...
>
> 开发机只缺第一条 → 1823 + 12（2026-09-09 实测）；基础安装路径（只装 `requirements*`，新用户实际得到的形态）
...
> pytest tests/ -q -rs | tail -1                                 # 无宿主：1823 passed, 12 skipped
> HERMES_SRC=/path/to/hermes-agent pytest tests/ -q | tail -1    # 有宿主：1835 passed
> HERMES_SRC=none pytest tests/ -q -rs | tail -1                 # 装了宿主也强制关掉，照旧 1823 passed, 12 skipped
>
...
> 会被自动发现，我们自己的生产机就是这样），宿主在场时不能套用 1823 + 12。上一实测基线：2026-09-07
...
> 上面代码块里的 `有宿主：1835 passed` 要**十三条轴同时齐备**才拿得到，宿主只是其中一条 ——
> 别把「装上宿主」当成「全绿」。基线：2026-09-07 在 v20.3.4 树的生产机独立全轴 venv 中，隔离 HOME、去掉 `.env`，
> 显式配置模型缓存和公开数据集后，实测到 **1743 passed、0 skipped**；本树全轴行 2026-09-09 生产机实测 1834/1。
