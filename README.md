...
| 维度 | 现状 |
|------|------|
| 用例总数 | **1084**（`pytest --collect-only` 实测） |
| 独立开发机 | 1072 通过 · **12 跳过** —— 缺宿主 Hermes 源码，有 git 工作区（实测） |
| 生产机沙箱 | 1083 通过 · **1 跳过** —— 有宿主 Hermes 源码，无 git 工作区（沙箱是白名单拷贝，不含 `.git`）。**这一行是按轴推导**：1084 减去 git 工作区那条轴门控的 1 条；最后一次真实沙箱实测是 **859 通过 · 1 跳过**，跑在 v20.0 提交树上（当时总数 860） |
| 全轴齐备 | 1084 全绿 —— **推导值，从未实测**：跳过有九条互不相干的轴，手上没有一台机器同时满足全部九条 |
...
> **为什么要把 1072 和 1083 都写出来**：同一份测试集在不同环境下跑出不同数字，只报其中一个都会误导读者。
> 1072 是本机实测；1083 是**按轴推导**（1084 减去 git 工作区那条轴）—— 最后一次真实沙箱实测是 859，
...
>
> 开发机缺第一条 → 1072 + 12；生产机沙箱缺第二条（白名单拷贝没有 `.git`）→ 1083 + 1。
> **两边各缺一条，所以「1084 全绿」我们从来没有实测过**，它只是推导值。上一版 README 把它写成
...
> pytest tests/ -q -rs | tail -1                                 # 无宿主：1072 passed, 12 skipped
> HERMES_SRC=/path/to/hermes-agent pytest tests/ -q | tail -1    # 有宿主：1084 passed
> HERMES_SRC=none pytest tests/ -q -rs | tail -1                 # 装了宿主也强制关掉，照旧 1072 passed, 12 skipped
> ```
>
> 「跳过」必须能被复现成「通过」，**反过来也必须成立**。机器上恰好装着宿主时（`/hermes/hermes-agent`
> 会被自动发现，我们自己的生产机就是这样），上面第一条命令跑出来其实是 1083 passed、1 skipped
> （**这个数是按轴推导的**：最后一次真实沙箱实测是 859 passed、1 skipped，跑在总数 860 的 v20.0 提交树上）——
> 剩下那 1 条卡在 git 工作区那条轴上（沙箱是白名单拷贝，树里没有 `.git`）。上面代码块里的
> `有宿主：1084 passed` **也是推导值**：1084 要九条轴同时齐备才拿得到，而我们手上没有这样一台机器。
