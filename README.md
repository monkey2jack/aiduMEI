...
| 生产机沙箱 | 1803 通过 · **9 跳过** —— **2026-09-08 生产机实测**（独立沙箱：部署树同款解释器、隔离 HOME、含 `.git`、不带 `.env`、公开数据集只读复用；峰值 RSS 408 MB、耗时 1 分 35 秒） |
| 全轴齐备 | 1812 通过 · **0 跳过** —— **2026-09-08 生产机实测**（独立全轴 venv：工具、extras、宿主源码、模型缓存与公开 LoCoMo 数据集齐备；峰值 RSS 530 MB、耗时 1 分 37 秒） |
...
| ④ | 生产机沙箱 · 宿主源码 · 生产 venv · 不带 `.env` | 1803 | 9 | 2026-09-08 实测（v20.4.0-alpha 候选树）：`ruff` ×3 + `mcp` ×5 + 本地嵌入模型缓存 ×1 |
| ⑤ | 全轴齐备 · 有 `.git` · 工具 · extras · 宿主 · 模型缓存 · 数据集 | 1812 | 0 | 2026-09-08 实测（v20.4.0-alpha 候选树，生产机独立全轴 venv）：无缺席轴 |

每行 `通过 + 跳过` 对应各自测试树的总数：①④⑤ 为本树（1812 条）2026-09-08 实测（分别在开发机、生产机独立沙箱、生产机独立全轴 venv）；②③ 为 1499 条历史基线，已被 ①④⑤ 取代。历史对照的用途和测量规则见 [docs/TESTING.md](docs/TESTING.md#历史矩阵的口径)，公开候选未复测的环境不能沿用旧结果冒充新测量。
...
> **为什么要把 1800 和 1803 都写出来**：1800 是本树开发环境 2026-09-08 实测（缺宿主 ×12）；1803 是同一天同一棵树在生产机独立沙箱的实测（缺 `ruff` ×3、`mcp` ×5、本地嵌入模型缓存 ×1）——两者的跳过轴不同，数字必须与环境、日期和测试树一起读。
...
> 全轴齐备（`.git`、工具、extras、宿主、模型缓存与数据集）→ **1812 + 0**（2026-09-08 生产机实测）。
>
...
> 在本树的生产机独立沙箱上跑出来是 1803 passed、9 skipped（2026-09-08 实测，不带 `.env`），那 9 条卡在 `ruff` ×3、
