...
| 维度 | 现状 |
|------|------|
| 用例总数 | **2144**（`pytest --collect-only` 实测，2026-09-19，v21.2-dev 本树）＝ **行为用例 1931（产品代码直测）+ 脚本/钩子行为 70 + 守卫用例 142（文档/口径/结构）**。三桶口径与名单见 `scripts/count_test_kinds.py`，可一键复算——v20.5.1 起头条不再用混合数（外部审计 C-1） |
...
> HERMES_SRC=/path/to/hermes-agent pytest tests/ -q | tail -1    # 有宿主：2144 passed
> HERMES_SRC=none pytest tests/ -q -rs | tail -1                 # 装了宿主也强制关掉，照旧 2131 passed, 12 skipped
> ```
>
> 上面代码块里的 `有宿主：2144 passed` 要**十三条轴同时齐备**才拿得到，宿主只是其中一条 —— 别把「装上宿主」当成「全绿」。
