...
| 维度 | 现状 |
|------|------|
| 用例总数 | **1552**（`pytest --collect-only` 实测） |
| 独立开发机 | 1540 通过 · **12 跳过** —— 缺宿主 Hermes 源码，有 git 工作区（实测） |
| 生产机沙箱 | 1549 通过 · **3 跳过** —— **2026-08-28 生产机实测**（白名单拷贝剔掉 `.git`，且不给 lint 工具；`pytest -rs` 打出的跳过理由逐条对得上：`ruff` 轴 2 条 + git 工作区轴 1 条）。有宿主 Hermes 源码。上一次真实沙箱实测是 **859 通过 · 1 跳过**，跑在 v20.0 提交树上（当时总数 860）—— 中间几版这一行都是**按轴推导**，本版起是实测 |
| 全轴齐备 | 1552 全绿 · 0 跳过 —— **待当前树在生产机重测**；上一基线为 1499 全绿 · 0 跳过（2026-08-28 生产机候选树实测） |
...
| # | 环境 | 通过 | 跳过 | 跳过归因 |
|---|------|-----:|-----:|---------|
| ① | 独立开发机 · 完整 extras | 1540 | 12 | 宿主 Hermes 源码缺席 ×12 |
| ② | 干净克隆 · **无配置** · 有 `.git`（≈ 第一次拿到本项目的人） | 1497 | 2 | `ruff` 未安装 ×2 |
| ③ | 干净克隆 · **带生产配置** · 有 `.git`（重排可达） | 1497 | 2 | `ruff` 未安装 ×2 |
| ④ | 生产部署树形态 · 带配置 · **无 `.git`** | 1549 | 3 | `ruff` ×2 + 不在 git 工作区 ×1 |
| ⑤ | 全轴齐备 · 带配置 · 有 `.git` · 旁挂 `ruff` | **待实测** | **0** | —— |

①~④ 的已测行满足 `通过 + 跳过 = 1552`；⑤ 必须重新实测后填数，不许沿用旧日期改数字 ——
...
>
> 上表的 1552/1540/12 跑在完整环境下：`regex`、`nltk`、`numpy`、`qdrant_client`、
...
> **为什么要把 1540 和 1549 都写出来**：同一份测试集在不同环境下跑出不同数字，只报其中一个都会误导读者。
...
>
> 开发机缺第一条 → 1540 + 12；生产机沙箱缺第二条（白名单拷贝没有 `.git`）→ 1549 + 3。
>
...
> pytest tests/ -q -rs | tail -1                                 # 无宿主：1540 passed, 12 skipped
> HERMES_SRC=/path/to/hermes-agent pytest tests/ -q | tail -1    # 有宿主：1552 passed
> HERMES_SRC=none pytest tests/ -q -rs | tail -1                 # 装了宿主也强制关掉，照旧 1540 passed, 12 skipped
> ```
>
> 「跳过」必须能被复现成「通过」，**反过来也必须成立**。机器上恰好装着宿主时（`/hermes/hermes-agent`
> 会被自动发现，我们自己的生产机就是这样），上面第一条命令跑出来其实是 1549 passed、3 skipped
> （**这个数是按轴推导的**：最后一次真实沙箱实测是 859 passed、1 skipped，跑在总数 860 的 v20.0 提交树上）——
> 剩下那 1 条卡在 git 工作区那条轴上（沙箱是白名单拷贝，树里没有 `.git`）。上面代码块里的
> `有宿主：1552 passed` 要十三条轴同时齐备才拿得到 —— 上一基线是 2026-08-28 在生产机候选树上**实测到**
