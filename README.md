...
| 维度 | 现状 |
|------|------|
| 用例总数 | **769**（`pytest --collect-only` 实测） |
| 独立开发机 | 757 通过 · **12 跳过** —— 跳过项需宿主 Hermes 源码在场，纯净环境下无法执行 |
| 完整环境 | **769 全绿**（Hermes 源码在场时 12 个跳过项全部执行并通过，生产实跑核验） |
...
> **为什么要把 757 和 769 都写出来**：同一份测试集在不同环境下跑出不同数字，只报其中一个都会误导读者。
...
>
> ```bash
> pytest tests/ -q -rs | tail -1                                 # 无宿主：757 passed, 12 skipped
> HERMES_SRC=/path/to/hermes-agent pytest tests/ -q | tail -1    # 有宿主：769 passed
> HERMES_SRC=none pytest tests/ -q -rs | tail -1                 # 装了宿主也强制关掉，照旧 757 passed, 12 skipped
> ```
>
> 「跳过」必须能被复现成「通过」，**反过来也必须成立**。机器上恰好装着宿主时（`/hermes/hermes-agent`
> 会被自动发现，我们自己的生产机就是这样），上面第一条命令跑出来其实是 769 passed、0 skipped ——
