...
| 维度 | 现状 |
|------|------|
| 用例总数 | **1290**（`pytest --collect-only` 实测） |
| 独立开发机 | 1278 通过 · **12 跳过** —— 缺宿主 Hermes 源码，有 git 工作区（实测） |
| 生产机沙箱 | 1289 通过 · **1 跳过** —— 有宿主 Hermes 源码，无 git 工作区（沙箱是白名单拷贝，不含 `.git`）。**这一行是按轴推导**：1290 减去 git 工作区那条轴门控的 1 条；最后一次真实沙箱实测是 **859 通过 · 1 跳过**，跑在 v20.0 提交树上（当时总数 860） |
| 全轴齐备 | 1290 全绿 · 0 跳过 —— **2026-08-26 生产机候选树实测**（bundle 克隆含 `.git`，十一条轴同时齐备；同日 v20.2.1 候选树实测 1281 全绿） |
...
> **为什么要把 1278 和 1289 都写出来**：同一份测试集在不同环境下跑出不同数字，只报其中一个都会误导读者。
> 1278 是本机实测；1289 是**按轴推导**（1290 减去 git 工作区那条轴）—— 最后一次真实沙箱实测是 859，
...
>
> 开发机缺第一条 → 1278 + 12；生产机沙箱缺第二条（白名单拷贝没有 `.git`）→ 1289 + 1。
>
...
> pytest tests/ -q -rs | tail -1                                 # 无宿主：1278 passed, 12 skipped
> HERMES_SRC=/path/to/hermes-agent pytest tests/ -q | tail -1    # 有宿主：1290 passed
> HERMES_SRC=none pytest tests/ -q -rs | tail -1                 # 装了宿主也强制关掉，照旧 1278 passed, 12 skipped
> ```
>
> 「跳过」必须能被复现成「通过」，**反过来也必须成立**。机器上恰好装着宿主时（`/hermes/hermes-agent`
> 会被自动发现，我们自己的生产机就是这样），上面第一条命令跑出来其实是 1289 passed、1 skipped
> （**这个数是按轴推导的**：最后一次真实沙箱实测是 859 passed、1 skipped，跑在总数 860 的 v20.0 提交树上）——
> 剩下那 1 条卡在 git 工作区那条轴上（沙箱是白名单拷贝，树里没有 `.git`）。上面代码块里的
> `有宿主：1290 passed` 要十一条轴同时齐备才拿得到 —— 2026-08-26 在生产机候选树上**实测到**
> （总数 1290，十一轴齐备、0 跳过）。但别把「装上宿主」当成「全绿」：宿主只是十一条轴里的一条。
