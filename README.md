...
| 生产机沙箱 | 1570 通过 · **3 跳过**（按轴推导：1573 − git 轴 3；**当前树尚未生产机复测**，复测后按实测改）—— 白名单拷贝剔掉 `.git` 且不给 lint 工具，跳过理由逐条归因：`ruff` 轴 2 条 + git 工作区轴 1 条 |
| 全轴齐备 | 1573 全绿 · 0 跳过（**按轴推导**，待当前树在生产机重测；上一实测基线 1499/0，2026-08-28） |
...
| ④ | 生产部署树形态 · 带配置 · **无 `.git`** | 1570 | 3 | `ruff` ×2 + 不在 git 工作区 ×1（按轴推导，待复测） |
...
> **为什么要把 1561 和 1570 都写出来**：同一份测试集在不同环境下跑出不同数字，只报其中一个都会误导读者。
...
>
> 开发机缺第一条 → 1561 + 12；生产机沙箱缺第二条（白名单拷贝没有 `.git`）→ 1570 + 3。
>
...
> 会被自动发现，我们自己的生产机就是这样），上面第一条命令跑出来其实是 1570 passed、3 skipped（按轴推导，待生产机复测）
