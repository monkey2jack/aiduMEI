...
模块边界与演进史见 [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md)（v14 时代历史快照，v20.4.1a 起归档入 docs/；现行形态以 [AGENTS.md](AGENTS.md) 数据流为准）。
...
| 维度 | 现状 |
|------|------|
| 用例总数 | **1837**（`pytest --collect-only` 实测，2026-09-09，v20.4.1a 本树） |
| 独立开发机 | 1825 通过 · **12 跳过** —— **2026-09-09 实测**（v20.4.1a 本树，Python 3.12；完整 extras + 模型缓存，只缺 Hermes 宿主） |
| 基础安装路径 | 1812 通过 · **25 跳过** —— 只装 `requirements.txt` + `requirements-dev.txt`（**2026-09-09 生产机干净 venv 实测**，v20.4.1a 本树，Python 3.12） |
| 生产机沙箱 | 1826 通过 · **11 跳过** —— **2026-09-09 生产机实测**（v20.4.1a 本树，独立沙箱 venv：宿主源码在场、不带 `.env`、无 ruff/mcp/fastembed；缺 `ruff` ×3、`mcp` ×7、备胎模型缓存 ×1） |
| 全轴齐备 | 1835 通过 · **1 跳过** —— **2026-09-09 生产机实测**（v20.4.1a 本树，独立全轴 venv：工具、extras、宿主源码、模型缓存与公开 LoCoMo 数据集齐备；那 1 跳过为本树新增用例的条件轴） |
...
| # | 环境 | 通过 | 跳过 | 跳过归因 |
|---|------|-----:|-----:|---------|
| ① | 独立开发机 · 完整 extras + 模型缓存 | 1825 | 12 | 2026-09-09 实测（v20.4.1a 本树；只缺宿主 Hermes ×12） |
...
| ④ | 生产机沙箱 · 宿主源码 · 独立沙箱 venv · 不带 `.env` | 1826 | 11 | 2026-09-09 实测（v20.4.1a 本树）：`ruff` ×3 + `mcp` ×7 + 备胎模型缓存 ×1 |
| ⑤ | 全轴齐备 · 有 `.git` · 工具 · extras · 宿主 · 模型缓存 · 数据集 | 1835 | 1 | 2026-09-09 实测（v20.4.1a 本树，生产机独立全轴 venv）：1 条本树新增条件轴 |
...
> **为什么要把 1825 和 1826 都写出来**：1825 是本树开发环境 2026-09-09 实测（缺宿主 ×12）；1826 是本树生产机独立沙箱 2026-09-09 实测（宿主在场，缺 `ruff` ×3、`mcp` ×7、备胎模型缓存 ×1）——两者的跳过轴不同，数字必须与环境、日期和测试树一起读。
...
> pytest tests/ -q -rs | tail -1                                 # 无宿主：1825 passed, 12 skipped
> HERMES_SRC=/path/to/hermes-agent pytest tests/ -q | tail -1    # 有宿主：1837 passed
> HERMES_SRC=none pytest tests/ -q -rs | tail -1                 # 装了宿主也强制关掉，照旧 1825 passed, 12 skipped
>
...
> 会被自动发现，我们自己的生产机就是这样），宿主在场时不能套用 1825 + 12。上一实测基线：2026-09-07
> 在本树的生产机独立沙箱上跑出来是 1826 passed、11 skipped（2026-09-09 实测，v20.4.1a 本树，不带 `.env`），那 11 条卡在 `ruff` ×3、`mcp` ×7、备胎模型缓存 ×1——
> `mcp` extra ×2 两条轴上（沙箱用生产 venv，不装 lint 工具与可选 extra；模型缓存和公开基准数据已显式配置；
> 本树 mcp 轴已增至 ×7，沙箱行待复测）。
> 上面代码块里的 `有宿主：1837 passed` 要**十三条轴同时齐备**才拿得到，宿主只是其中一条 ——
