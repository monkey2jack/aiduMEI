...
| 全轴齐备 | 1270 全绿 · 0 跳过为按轴推导的目标值，自审补项后待生产沙箱复测；最近一次全轴实测是 1267 全绿（**2026-08-26 生产机候选树实测**，当时总数 1267、十一条轴）。更早这一行是推导值，标注为「从未实测」长达数个版本 |
...
> `有宿主：1270 passed` 要十一条轴同时齐备才拿得到 —— 自审补项后待生产沙箱复测；上一次全轴实测
> 是 1267（2026-08-26 生产机候选树，十一轴齐备、0 跳过）。但别把「装上宿主」当成「全绿」：宿主只是十一条轴里的一条。
