...
| 维度 | 现状 |
|------|------|
| 用例总数 | **1581**（`pytest --collect-only` 实测，2026-09-02） |
| 独立开发机 | 1569 通过 · **12 跳过** —— 缺宿主 Hermes 源码，有 git 工作区（**2026-09-02 实测**，v20.3.2 树） |
| 基础安装路径 | 1550 通过 · **31 跳过** —— 只装 `requirements.txt` + `requirements-dev.txt`，**这是新用户实际会得到的数**（**2026-09-02 干净 venv 实测**，Python 3.12） |
| 生产机沙箱 | **待复测**（按轴推导值为 1576/5；上一实测 1568/5 是总数 1573 的树，2026-09-01） |
| 全轴齐备 | **待复测**（按轴推导值为 1581/0；上一实测 1573/0 是总数 1573 的树，2026-09-02 生产机五轴齐备实测） |
...
| # | 环境 | 通过 | 跳过 | 跳过归因 |
|---|------|-----:|-----:|---------|
| ① | 独立开发机 · 完整 extras | 1569 | 12 | 宿主 Hermes 源码缺席 ×12（2026-09-02 实测，v20.3.2 树） |
...
| ④ | 生产机沙箱 · 带配置 · 宿主源码 · local_embed_cache | 待复测 | 5 | 推导 1576/5；上一实测 1568/5（总数 1573 的树，2026-09-01） |
| ⑤ | 全轴齐备 · 带配置 · 有 `.git` · `ruff` · `mcp` extra · 宿主 · 备胎模型 | 待复测 | **0** | 推导 1581/0；上一实测 1573/0（2026-09-02 生产机五轴齐备） |

已测行满足 `通过 + 跳过 = 1581`（②③ 是总数 1499 时代的基线，已标注日期）；未实测的行必须重新实测后填数，不许沿用旧日期改数字 ——
...
>
> 上表的 1581/1569/12 跑在完整环境下：`regex`、`nltk`、`numpy`、`qdrant_client`、
...
> **为什么要把 1569 和 1568 都写出来**：同一份测试集在不同环境下跑出不同数字，只报其中一个都会误导读者。
...
>
> 开发机缺第一条 → 1569 + 12；基础安装路径（只装 `requirements*`，新用户实际得到的形态）
> → **1550 + 31**（2026-09-02 干净 venv 实测）；生产机沙箱 → 1568 + 5（上一基线，待在新树复测）；
> 五轴齐备（`.git` + `ruff` + `mcp` extra + 宿主 + 备胎模型缓存）→ 1573 + 0（同一基线，待复测）。
>
...
> pytest tests/ -q -rs | tail -1                                 # 无宿主：1569 passed, 12 skipped
> HERMES_SRC=/path/to/hermes-agent pytest tests/ -q | tail -1    # 有宿主：1581 passed
> HERMES_SRC=none pytest tests/ -q -rs | tail -1                 # 装了宿主也强制关掉，照旧 1569 passed, 12 skipped
>
> # 基础安装路径（只装 requirements*，不装任何可选组）—— 新用户实际会跑到的形态
> pip install -r requirements.txt -r requirements-dev.txt
> pytest tests/ -q -rs | tail -1                                 # 基础路径：1550 passed, 31 skipped
> ```
>
