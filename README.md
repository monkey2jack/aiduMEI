...
| 维度 | 现状 |
|------|------|
| 用例总数 | **1200**（`pytest --collect-only` 实测） |
| 独立开发机 | 1188 通过 · **12 跳过** —— 缺宿主 Hermes 源码，有 git 工作区（实测） |
| 生产机沙箱 | 1199 通过 · **1 跳过** —— 有宿主 Hermes 源码，无 git 工作区（沙箱是白名单拷贝，不含 `.git`）。**这一行是按轴推导**：1200 减去 git 工作区那条轴门控的 1 条；最后一次真实沙箱实测是 **859 通过 · 1 跳过**，跑在 v20.0 提交树上（当时总数 860） |
| 全轴齐备 | 1200 全绿 · 0 跳过 —— **按轴推导**（v20.1-dev 新增用例后尚未全轴复测）；最后一次全轴实测是 **1112 全绿 · 0 跳过**（2026-08-24 生产实机，当时总数 1112，九条轴同时齐备：宿主源码在场、有 `.git`、基准依赖与数据集齐全）。更早这一行是推导值，标注为「从未实测」长达数个版本 |
...
> **为什么要把 1188 和 1199 都写出来**：同一份测试集在不同环境下跑出不同数字，只报其中一个都会误导读者。
> 1188 是本机实测；1199 是**按轴推导**（1200 减去 git 工作区那条轴）—— 最后一次真实沙箱实测是 859，
...
>
> 开发机缺第一条 → 1188 + 12；生产机沙箱缺第二条（白名单拷贝没有 `.git`）→ 1199 + 1。
>
> **「全轴全绿」在 2026-08-24 实测到过一次**（当时总数 1112，生产实机九轴齐备、0 跳过）；v20.1-dev 新增用例后，当前总数 1200 的全绿回到**推导值**，待下一次全轴实机复测转正。这一行的来历值得留着：
...
> pytest tests/ -q -rs | tail -1                                 # 无宿主：1188 passed, 12 skipped
> HERMES_SRC=/path/to/hermes-agent pytest tests/ -q | tail -1    # 有宿主：1200 passed
> HERMES_SRC=none pytest tests/ -q -rs | tail -1                 # 装了宿主也强制关掉，照旧 1188 passed, 12 skipped
> ```
>
> 「跳过」必须能被复现成「通过」，**反过来也必须成立**。机器上恰好装着宿主时（`/hermes/hermes-agent`
> 会被自动发现，我们自己的生产机就是这样），上面第一条命令跑出来其实是 1199 passed、1 skipped
> （**这个数是按轴推导的**：最后一次真实沙箱实测是 859 passed、1 skipped，跑在总数 860 的 v20.0 提交树上）——
> 剩下那 1 条卡在 git 工作区那条轴上（沙箱是白名单拷贝，树里没有 `.git`）。上面代码块里的
> `有宿主：1200 passed` 要九条轴同时齐备才拿得到 —— 全轴全绿最后一次实测是 2026-08-24 生产实机
> （当时总数 1112，九轴齐备、0 跳过）。但别把「装上宿主」当成「全绿」：宿主只是九条轴里的一条。
