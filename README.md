...
**一台引擎，两种动力，自动换挡** —— 市面上第一个把「本地备胎」做成全链路自动降挡的开源记忆系统：

| | ☁️ 云挡（Full） | 🔋 备胎挡（Lite） |
|---|---|---|
| 动力 | LLM 蒸馏 + 云端嵌入全性能 | 纯本地 ONNX 嵌入（512 维小模型）+ 确定性抽取 + 提取式整合 |
| 成本 | 正常计费 | **零 token · 零网络 · 零外部依赖** |
| 何时用 | 平时 | 嵌入服务断供的每一秒 —— 你不用做任何事 |

- **自动降挡**：嵌入服务连续失败触发熔断切换；且**单次查询内就地换腿**——云腿当场炸掉，这一次请求就落到本地索引，无感顺滑，不是下一个用户才享受降挡。
- **自动升挡**：半开探测拿真实流量试探，连续成功才回云挡（假恢复骗不动挡位）；降挡期间欠下的 LLM 蒸馏自动重放补算，一条不丢。
- **挡位诚实**：`/search` 响应带 `engine_mode`（按本次实际用的腿报告）；lite 分数口径明示；升降挡进事件账本——哪段时间跑在备胎上，审计可查。注意字段形态差异：lite 挡的 `/add` 受理响应是写路径契约（`status` / `action: deferred_distillation` / `engine_mode`），**不含** `/search` 的召回判语字段族（`recall_verdict` 等）——欠账受理不是召回，调用方勿按 `/search` 契约解析。
- **生产实测**：断供演练在真实生产环境全链验证——封锁嵌入端点 → 三连失败自动降挡 → 断供期写入照落且**语义可召回**（`vector_leg=local` 实证）→ 恢复自动升挡 → 欠账消化归零。
- **诚实标注**：lite 是保命档不是平替——20 条真实查询对照，本地小模型与云模型的 top5 排序重叠约 9%（口径含原文向量稀释与模型排序分歧）。断供时搜得到该搜到的，排序品质明写不如云挡。
- **三档随你选**（v20.2.3）：自动挡好，但不该是唯一选项。`AIDUMEI_ENGINE_MODE` 一行切换 **云端档**（只用云，省下备胎那 151MB）/ **自动挡**（默认，断供无感续跑）/ **本地档**（零 token、零外部网络、不需要任何密钥）。两条腿是独立开关，不是三选一的死结构。
- **LLM 蒸馏腿同样有挡位**（v20.2.2）：蒸馏服务断供时写入秒级降为确定性直写——原文、硬事实、云向量照落，内容照样可召回，欠的只是蒸馏精修且账目可查；连续失败自动降挡（挡内不再逐请求撞超时），恢复经半开真实写入探测自动升挡；传输层盲重试已掐（网关的 Retry-After 不再把单次写入挂几分钟）。响应带 `distillation` 注记，`/health` 有 `llm_gear` 探针。

裸装（不配任何云服务密钥）时它天然一直跑在备胎挡——**开箱即用的零依赖记忆库**；配上密钥自动升挡。一个包，自适应。
...
| 维度 | 现状 |
|------|------|
| 用例总数 | **1332**（`pytest --collect-only` 实测） |
| 独立开发机 | 1320 通过 · **12 跳过** —— 缺宿主 Hermes 源码，有 git 工作区（实测） |
| 生产机沙箱 | 1331 通过 · **1 跳过** —— 有宿主 Hermes 源码，无 git 工作区（沙箱是白名单拷贝，不含 `.git`）。**这一行是按轴推导**：1332 减去 git 工作区那条轴门控的 1 条；最后一次真实沙箱实测是 **859 通过 · 1 跳过**，跑在 v20.0 提交树上（当时总数 860） |
| 全轴齐备 | 1332 全绿 · 0 跳过 —— **2026-08-27 生产机候选树实测**（bundle 克隆含 `.git`，十一条轴同时齐备） |
...
> **为什么要把 1320 和 1331 都写出来**：同一份测试集在不同环境下跑出不同数字，只报其中一个都会误导读者。
> 1320 是本机实测；1331 是**按轴推导**（1332 减去 git 工作区那条轴）—— 最后一次真实沙箱实测是 859，
...
>
> 开发机缺第一条 → 1320 + 12；生产机沙箱缺第二条（白名单拷贝没有 `.git`）→ 1331 + 1。
>
...
> pytest tests/ -q -rs | tail -1                                 # 无宿主：1320 passed, 12 skipped
> HERMES_SRC=/path/to/hermes-agent pytest tests/ -q | tail -1    # 有宿主：1332 passed
> HERMES_SRC=none pytest tests/ -q -rs | tail -1                 # 装了宿主也强制关掉，照旧 1320 passed, 12 skipped
> ```
>
> 「跳过」必须能被复现成「通过」，**反过来也必须成立**。机器上恰好装着宿主时（`/hermes/hermes-agent`
> 会被自动发现，我们自己的生产机就是这样），上面第一条命令跑出来其实是 1331 passed、1 skipped
> （**这个数是按轴推导的**：最后一次真实沙箱实测是 859 passed、1 skipped，跑在总数 860 的 v20.0 提交树上）——
> 剩下那 1 条卡在 git 工作区那条轴上（沙箱是白名单拷贝，树里没有 `.git`）。上面代码块里的
> `有宿主：1332 passed` 要十一条轴同时齐备才拿得到 —— 2026-08-26 在生产机候选树上**实测到**
> （总数 1332，十一轴齐备、0 跳过）。但别把「装上宿主」当成「全绿」：宿主只是十一条轴里的一条。
