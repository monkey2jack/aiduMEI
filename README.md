...
> **v20.0 · 记忆域隔离 · 可复现评测 · 后端契约与数据生命线。** 架构版。版本号自本版起回归两段式，运行时不再设神话代号——诸神留在谱系里作历史。v19 之前，整条记忆链路实际上只有**一个隐式记忆池**：「这条记忆属于谁、属于哪个领域」靠 `source`/`agent_id` 这类**渠道标记**兼职表达。但**渠道不是所有权**——两个互不相干的记忆域一旦撞上同一个键，后写的那条会直接盖掉先写的，**两边都不报错**，先写的就那么没了。所以 v20 的主题不是「加一个 bank 字段」，而是把作用域从一种**约定**变成一份**契约**：新增 `ducky/bank_contract.py` 确立 `(user_id, bank_id)` 二维作用域，写、查、删、恢复、统计、反馈、后台任务、事件账本、Instinct→Skill 毕业链、persona 画像——每一条**在线读写**路径都必须显式说出自己在哪个域上工作（全库维护作业等三处已知例外见〈已知例外与本版不覆盖〉），非法作用域在**取数之前**就抛错，绝不静默降级成全库扫描。读侧统一一个形状：**命名域下推过滤且复筛，默认域保持 v19 形状但复筛掉命名域的点**（存量向量没有 bank 字段，默认域不下推正是为了它们）；迁移全程 additive，**存量数据一行都不改、一行都不删**，老数据一律落 `default` 域，键形状与 v19 逐字相同。同版新增 `benchmarks/` 把评测固定成**可复现协议**（数据集、judge、seed、文件哈希全部锁死留证；适配器走真实 HTTP 契约不走进程内捷径；oracle 只作检索上限诊断不进 headline），新增 `ducky/vector_backend.py` 把向量后端从写死变成**契约**（Qdrant 仍是默认，sqlite-vec 只作可回退的影子 POC，**不把「不可逆换库」与「数据域迁移」绑在同一次发布里**）；`/health`、`/metrics`、`/search` trace 补齐 bank / backend / 降级证据，组件故障一律进 degraded / trace，**不许伪装成空结果**——「没搜到」和「搜挂了」长得一模一样，是这个项目反复付过学费的失败形态。

> **v19.5.0 · 雅典娜——脱敏闸门 · 把铁律变成不可绕过的程序。** 纪律版，不改任何运行时行为，改的是**「什么情况下才允许发布」**。连续两版栽在同一件事上：脱敏做了，但做没做到位，全靠人的记性和自觉——写在文档里的铁律，人会漏，**而漏了以后没有任何东西会红**。更麻烦的是这类工具的失败是**沉默的**：词表少配一个词，扫描照跑、报告全绿、脏包上了公开索引，事后才发现；它不报错、不崩溃，只是安静地发放一张「已经检查过了」的凭证。**一个坏掉的扫描器和一个干净的项目，报出来的东西一模一样：都是「0」。** 所以本版真正要解决的不是「能不能扫出来」，而是**「当扫描器坏掉时，它会不会老老实实地失败」**。新增的 `scripts/release_scan.py` 覆盖七个公开面（新增「⑥ 包索引渲染面」——元数据渲染出来的项目网页正文，**不下载就能看见**），并立三条硬约束：**词表一律外置于仓库之外**（词表里装的正是要藏的东西，写进仓库等于为了防泄露而制造一次泄露）；**空词表拒绝运行而非放行**（退出码 2，绝不输出那个与「真的干净」无法区分的 0）；**负向对照焊进代码**——每次真扫前先在内置合成样本上三向验证（脏样本必须报警、干净样本必须不报、豁免不得越行），任何一边不符即整轮作废。配套的 `tests/test_release_hygiene.py` 用 18 条守卫盯着闸门本身，核心一条是把扫描逻辑打坏成**瞎子**（什么都不报）、**疯子**（什么都报）、**漏勺**（豁免溢出整个文件）三种故障，自检必须每一种都抓到——**一个永远通过的自检等于没有自检**。豁免出口刻意做得很窄：**只认命中所在的那一行**，无文件级、更无目录级豁免，且豁免仍逐条出现在报告里——从**失败计数**里消失，不从**视野**里消失。

> **v19.4.3 · 雅典娜——发布卫生 · 发行包也是公开面。** v19.4.2 的等价版，**零行为变更**：可执行逻辑与 v19.4.2 完全一致，差异仅在注释、docstring 与版本号本身，升级无需任何适配。做这一版的原因只有一条——v19.4.2 的源码注释与 docstring 里残留了内部部署环境的描述性文字，而**发布到包索引上的同一个版本号永久不可覆盖、不可修改**。在所有对外可见的面里，提交信息能改、标签能重打、发布正文能编辑，**只有已上传的发行包不能**；顺序因此不是建议而是约束——**扫描必须发生在上传之前**。本版把这道卡点写进发布流程：发行包必须解包实扫，且扫描器本身必须先在**已知命中的对象**上验证有效（负向对照），那个「0 命中」才作数——**一个从未报出过任何命中的扫描器，它的 0 不是证据**。
...
| 维度 | 现状 |
|------|------|
| 用例总数 | **1111**（`pytest --collect-only` 实测） |
| 独立开发机 | 1099 通过 · **12 跳过** —— 缺宿主 Hermes 源码，有 git 工作区（实测） |
| 生产机沙箱 | 1110 通过 · **1 跳过** —— 有宿主 Hermes 源码，无 git 工作区（沙箱是白名单拷贝，不含 `.git`）。**这一行是按轴推导**：1111 减去 git 工作区那条轴门控的 1 条；最后一次真实沙箱实测是 **859 通过 · 1 跳过**，跑在 v20.0 提交树上（当时总数 860） |
| 全轴齐备 | 1111 全绿 · 0 跳过 —— **2026-08-24 生产实机实测**：九条轴在那台机器上同时齐备（宿主源码在场、有 `.git`、基准依赖与数据集齐全）。在此之前这一行是推导值，标注为「从未实测」长达数个版本 |
...
> **为什么要把 1099 和 1110 都写出来**：同一份测试集在不同环境下跑出不同数字，只报其中一个都会误导读者。
> 1099 是本机实测；1110 是**按轴推导**（1111 减去 git 工作区那条轴）—— 最后一次真实沙箱实测是 859，
...
>
> 开发机缺第一条 → 1099 + 12；生产机沙箱缺第二条（白名单拷贝没有 `.git`）→ 1110 + 1。
>
> **「1111 全绿」现在是实测值了**（2026-08-24，生产实机九轴齐备、0 跳过）。这一行的来历值得留着：
> 更早的一版 README 把它写成「生产实跑核验」，而它恰恰被自己引用的那次生产实跑当场证伪；
> 于是改成「推导值，从未实测」，并配了一条**会随环境失效**的断言把这句话钉住 ——
> 真有一天齐备了，那条断言就该红。它在 2026-08-24 按设计红了，这一行才换成实测值。
...
> pytest tests/ -q -rs | tail -1                                 # 无宿主：1099 passed, 12 skipped
> HERMES_SRC=/path/to/hermes-agent pytest tests/ -q | tail -1    # 有宿主：1111 passed
> HERMES_SRC=none pytest tests/ -q -rs | tail -1                 # 装了宿主也强制关掉，照旧 1099 passed, 12 skipped
> ```
>
> 「跳过」必须能被复现成「通过」，**反过来也必须成立**。机器上恰好装着宿主时（`/hermes/hermes-agent`
> 会被自动发现，我们自己的生产机就是这样），上面第一条命令跑出来其实是 1110 passed、1 skipped
> （**这个数是按轴推导的**：最后一次真实沙箱实测是 859 passed、1 skipped，跑在总数 860 的 v20.0 提交树上）——
> 剩下那 1 条卡在 git 工作区那条轴上（沙箱是白名单拷贝，树里没有 `.git`）。上面代码块里的
> `有宿主：1111 passed` 要九条轴同时齐备才拿得到 —— 2026-08-24 在生产实机上**已经实测到**
> （九轴齐备、0 跳过）。但别把「装上宿主」当成「全绿」：宿主只是九条轴里的一条。
