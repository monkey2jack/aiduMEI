...
| 维度 | 现状 |
|------|------|
| 用例总数 | **1469**（`pytest --collect-only` 实测） |
| 独立开发机 | 1457 通过 · **12 跳过** —— 缺宿主 Hermes 源码，有 git 工作区（实测） |
| 生产机沙箱 | 1466 通过 · **3 跳过** —— **2026-08-28 生产机实测**（白名单拷贝剔掉 `.git`，且不给 lint 工具；`pytest -rs` 打出的跳过理由逐条对得上：`ruff` 轴 2 条 + git 工作区轴 1 条）。有宿主 Hermes 源码。上一次真实沙箱实测是 **859 通过 · 1 跳过**，跑在 v20.0 提交树上（当时总数 860）—— 中间几版这一行都是**按轴推导**，本版起是实测 |
| 全轴齐备 | 1469 全绿 · 0 跳过 —— **2026-08-28 生产机候选树实测**（bundle 克隆含 `.git`，十二条轴同时齐备；第十二条轴靠 `pip install --target` 旁挂 `ruff`，**不写生产 venv**）。上一次全轴实测是 2026-08-27 的 **1440 全绿**，跑在 v20.2.4 的十一条轴上 —— 轴数与总数都变了，所以本版重测而不是把旧结论的数字改一改 |
...
>
> 上表的 1469/1457/12 跑在完整环境下：`regex`、`nltk`、`numpy`、`qdrant_client`、
...
> **为什么要把 1457 和 1466 都写出来**：同一份测试集在不同环境下跑出不同数字，只报其中一个都会误导读者。
> **1457 与 1466 都是实测**（2026-08-28，分别在公鸡与生产机上跑）；1469 是全轴齐备的实测值，
...
>
> 开发机缺第一条 → 1457 + 12；生产机沙箱缺第二条（白名单拷贝没有 `.git`）→ 1466 + 3。
>
...
> pytest tests/ -q -rs | tail -1                                 # 无宿主：1457 passed, 12 skipped
> HERMES_SRC=/path/to/hermes-agent pytest tests/ -q | tail -1    # 有宿主：1469 passed
> HERMES_SRC=none pytest tests/ -q -rs | tail -1                 # 装了宿主也强制关掉，照旧 1457 passed, 12 skipped
> ```
>
> 「跳过」必须能被复现成「通过」，**反过来也必须成立**。机器上恰好装着宿主时（`/hermes/hermes-agent`
> 会被自动发现，我们自己的生产机就是这样），上面第一条命令跑出来其实是 1466 passed、3 skipped
> （**这个数是按轴推导的**：最后一次真实沙箱实测是 859 passed、1 skipped，跑在总数 860 的 v20.0 提交树上）——
> 剩下那 1 条卡在 git 工作区那条轴上（沙箱是白名单拷贝，树里没有 `.git`）。上面代码块里的
> `有宿主：1469 passed` 要十二条轴同时齐备才拿得到 —— 2026-08-27 在生产机候选树上**实测到**
> （总数 1469，十二轴齐备、0 跳过）。但别把「装上宿主」当成「全绿」：宿主只是十二条轴里的一条。
