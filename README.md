...
## 🛡️ 安全边界：被第三方按我们自己的契约审了两轮（v20.2.4 / v20.2.5）
...
| `DELETE` | `/delete` | 按 ID 删除记忆（入参走 query；**响应三态**见下） |
| `GET` | `/health` | 健康检查 + 全探针诊断（含 `runtime_paths`：实际打开的数据/日志路径与可写性） |

> **删除是三态的，不是「成功/报错」两态**（v20.2.5）：`committed`→**200**、`partial`→**207**、`failed`→**500**。
> 207 是刻意选的 —— 它强制调用方注意到「不是完全成功」。响应里 `failed_layers` 是**本次真的失败**的层，
> `not_cleared` 是**删除链矩阵预声明豁免**的表，两者分开报，不混成一句「ok」。`partial` 时 WAL **不**标提交，
> 留在待重放状态。
>
> `/search` 的入参有硬边界：`limit` 取 `1..100`，`query` 有长度上限；空 `query` 返回
> `recall_verdict="empty_query"` 而**不是**随便给几条记忆。
