...
Write and search must use the same `user_id` and `bank_id`. `/search` returns an empty query verdict instead of random memories. Delete responses distinguish failed layers from intentionally exempt layers.
...
| 维度 | 现状 |
|------|------|
| 用例总数 | **1573**（`pytest --collect-only` 实测，2026-09-01） |
| 独立开发机 | 1561 通过 · **12 跳过** —— 缺宿主 Hermes 源码，有 git 工作区（**2026-09-01 实测**，v20.3.1 树） |
| 生产机沙箱 | 1568 通过 · **5 跳过** —— **2026-09-01 生产机实测**（沙箱含 Hermes 宿主轴 + local_embed_cache，连续三轮同值；跳过逐条归因）。上一实测基线 1499/0（2026-08-28） |
| 全轴齐备 | 1573 通过 · **0 跳过** —— **2026-09-02 生产机实测**（五轴齐备：`.git` + `ruff` + `mcp` extra + 宿主源码 + 备胎模型缓存；80.7s）。与按轴推导值 1573/0 一致，推导这次升格为实测 |
...
| # | 环境 | 通过 | 跳过 | 跳过归因 |
|---|------|-----:|-----:|---------|
| ① | 独立开发机 · 完整 extras | 1561 | 12 | 宿主 Hermes 源码缺席 ×12（2026-09-01 实测） |
...
| ④ | 生产机沙箱 · 带配置 · 宿主源码 · local_embed_cache | 1568 | 5 | 2026-09-01 实测（连续三轮） |
| ⑤ | 全轴齐备 · 带配置 · 有 `.git` · `ruff` · `mcp` extra · 宿主 · 备胎模型 | **1573** | **0** | 2026-09-02 生产机实测（五轴齐备，见下） |

①④⑤ 的已测行满足 `通过 + 跳过 = 1573`（②③ 是总数 1499 时代的基线，已标注日期）；未实测的行必须重新实测后填数，不许沿用旧日期改数字 ——
...
>
> 上表的 1573/1561/12 跑在完整环境下：`regex`、`nltk`、`numpy`、`qdrant_client`、
...
> **为什么要把 1561 和 1568 都写出来**：同一份测试集在不同环境下跑出不同数字，只报其中一个都会误导读者。
...
>
> 开发机缺第一条 → 1561 + 12；生产机沙箱（宿主 + local_embed_cache，无 `.git`）→ 1568 + 5；
> 五轴齐备（`.git` + `ruff` + `mcp` extra + 宿主 + 备胎模型缓存）→ **1573 + 0**（2026-09-02 生产机实测）。
>
> **「全轴全绿」是实测值**（2026-09-02 生产机，总数 1573、五轴齐备、0 跳过）。这一行的来历值得留着：
...
> pytest tests/ -q -rs | tail -1                                 # 无宿主：1561 passed, 12 skipped
> HERMES_SRC=/path/to/hermes-agent pytest tests/ -q | tail -1    # 有宿主：1573 passed
> HERMES_SRC=none pytest tests/ -q -rs | tail -1                 # 装了宿主也强制关掉，照旧 1561 passed, 12 skipped
> ```
>
> 「跳过」必须能被复现成「通过」，**反过来也必须成立**。机器上恰好装着宿主时（`/hermes/hermes-agent`
> 会被自动发现，我们自己的生产机就是这样），上面第一条命令跑出来就不是 1561 + 12 —— 2026-09-01
> 在生产机沙箱上跑出来是 1568 passed、5 skipped（连续三轮同值，实测）。剩下那 5 条卡在 `ruff` ×2、
> `mcp` extra ×2、备胎模型文件 ×1 三条轴上（沙箱用生产 venv，不装 lint 工具与可选 extra）。
> 上面代码块里的 `有宿主：1573 passed` 要**十三条轴同时齐备**才拿得到，宿主只是其中一条 ——
> 别把「装上宿主」当成「全绿」。2026-09-02 在生产机上为它单独建了一个带 `mcp` extra 的测试
> venv，五轴齐备后实测到 **1573 passed、0 skipped**。
