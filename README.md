...
| 维度 | 现状 |
|------|------|
| 用例总数 | **351**（`pytest --collect-only` 实测） |
| 独立开发机 | 339 通过 · **12 跳过** —— 跳过项需宿主 Hermes 源码在场，纯净环境下无法执行 |
| 完整环境 | **351 全绿**（Hermes 源码在场时 12 个跳过项全部执行并通过） |
...
> **为什么要把 339 和 351 都写出来**：同一份测试集在不同环境下跑出不同数字，只报其中一个都会误导读者。
