...
每行 `通过 + 跳过` 对应各自测试树的总数：① 为当前 1743 条的实测值；②③ 为 1499 条历史基线；④⑤ 为 1728 条历史基线。当前树未复测的环境不能沿用旧日期改数字 ——
...
> **为什么要把 1731 和 1723 都写出来**：前者是当前开发环境的实测值，后者是旧生产沙箱的实测基线；数字必须与环境、日期、测试树和证据类型一起读。
...
> → **1711 + 32**（2026-09-07 干净 venv 实测）；生产机沙箱 → 1723 + 5（2026-09-03）；
...
>
> ```bash
> # 装齐依赖和模型；无宿主形态已于 2026-09-07 实测；有宿主形态当前待复测
...
> 别把「装上宿主」当成「全绿」。2026-09-03 在生产机上为它单独建了一个带 `mcp` extra 的测试
