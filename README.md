...
| 全轴齐备 | **待复测**（按轴推导值为 1573/0；沙箱实测 1568+5 非 all-axes 形态——无 `.git` 与 lint 工具。上一实测基线 1499/0，2026-08-28） |
...
①~④ 的已测行满足 `通过 + 跳过 = 1573`；未实测的行必须重新实测后填数，不许沿用旧日期改数字 ——
...
> 会被自动发现，我们自己的生产机就是这样），上面第一条命令在带宿主与 local_embed_cache 的沙箱上跑出来是 1568 passed、5 skipped（2026-09-01 生产机实测，连续三轮同值）
> （**这个数是按轴推导的**：最后一次真实沙箱实测是 859 passed、1 skipped，跑在总数 860 的 v20.0 提交树上）——
> 剩下那 1 条卡在 git 工作区那条轴上（沙箱是白名单拷贝，树里没有 `.git`）。上面代码块里的
> `有宿主：1573 passed` 要十三条轴同时齐备才拿得到 —— 上一基线是 2026-08-28 在生产机候选树上**实测到**
