...
| 生产机沙箱 | 1841 通过 · **11 跳过** —— **2026-09-10 生产机实测**（v20.5.0a 本树，独立沙箱 venv：宿主源码在场、不带 `.env`、无 ruff/mcp/fastembed；缺 `ruff` ×3、`mcp` ×7、备胎模型缓存 ×1） |
...
> **为什么要把 1845 和 1841 都写出来**：1845 是本树开发环境 2026-09-10 实测（缺宿主 ×12）；1841 是上一树（fa0f73d）生产机独立沙箱 2026-09-10 实测（宿主在场，缺 `ruff` ×3、`mcp` ×7、备胎模型缓存 ×1）——两者的跳过轴不同，数字必须与环境、日期和测试树一起读。
...
> 在本树的生产机独立沙箱上跑出来是 1841 passed、11 skipped（2026-09-10 实测，v20.5.0a 本树 fa0f73d，不带 `.env`），那 11 条卡在 `ruff` ×3、`mcp` ×7、备胎模型缓存 ×1——
