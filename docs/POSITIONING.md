...
|---|:---:|:---:|:---:|
| 多租户 / 记忆域隔离 | **✅ `(user_id, bank_id)` 二维契约[^tenant-note]** | ✗ 明示为单机单智能体设计 | 部分支持 |
...
- **重排 ≠ 加权融合。** 加权融合是把已有的分数加一加；cross-encoder 重排是把问题和文档**放在一起重读一遍**再打分。长文档检索上这一步通常值 10–20 个点（**行业文献中 cross-encoder 对 bi-encoder 的惯例收益区间，非本引擎实测**——本引擎的实测数字等正式跑分，见 `benchmarks/RESULTS.md`）。
