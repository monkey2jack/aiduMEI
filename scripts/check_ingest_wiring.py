...
def diagnose(health: dict, *, require_judgment: bool = False) -> tuple[int, list[str], dict]:
    ...
    facts = {
        ...
        "distill_sessions_effective_24h": probes.get("distill_sessions_effective_24h"),
    ...
    dis_eff = facts.get("distill_sessions_effective_24h")
    if dis_sess is not None and dis_made is not None:
        lines.append(f"会话精华萃取：最近 24 小时经历 {dis_sess} 个会话 · 产出 {dis_made} 条精华")
        if dis_eff is not None:
            lines.append(f"   （其中 {dis_eff} 个够得着萃取门槛（≥3条））")
        if dis_ok is False:
            lines += [
                "",
                "⚠️  第三条线（会话精华萃取）疑似未触发或挂载异常：",
                f"   最近 24h 有 {dis_eff or dis_sess} 个会话够得着萃取门槛，却一条精华都没产出。",
