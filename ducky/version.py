...
v20.4.0 (alpha 阶段快照 · 开工 · 六方外审整改 · 2026-09-08)
    ...
    5. P2：CSP 去 unsafe-inline；SSH known_hosts 预置文档化；except 密度棘轮 601 只降不升；
       ...
       双语关键段（安装/安全/三探针）对齐。
    6. 用例总数 1743 → 1835（--collect-only）：三方审计 P0/P1 整改在 alpha 基础上再增 23 条；
       开发机本树 2026-09-09 实测 1823+12；干净 venv 1777+35（2026-09-08 alpha 树）；
       生产机独立全轴 2026-09-09 本树实测 1834+1；生产机独立沙箱换树后待复测
       （按轴推导 1827/9，上一实测基线 1803+9 @2026-09-08 alpha 树）。
    7. 生产热修 3219f8c4e 并回（2026-09-07 生产机直改、从未推远端）：两段 inline
       ...
       'self'，不放宽）；Vision 请求补 User-Agent 且版本号取本模块单源。三面守卫
       在 tests/test_v20_4_prod_hotfix_reconcile.py：HTML 无 inline、实测响应头无
       unsafe-inline、被引用 src 可服务。P2-11 只钉了 style 那一格是射程教训。
...
v20.3 (正式版 · 五方外审整改 · 一致性与底层 · 2026-09-03)
    ...
    11. 收口后补修（同日下午，站长指令「能解决的一次性解决」）：悬挂事务探测按**请求上下文戳**区分
       嵌套借用与真悬挂（生产首日报的 2 次是 write_fact INSERT 在途 → 治理钩子再借 —— 探测器假红，
       改后两轮 e2e 计数 0）；水位阈值有了容量依据（docs/CAPACITY.md，生产设 3000，默认 800 不动）；
       跳过轴普查的备胎探针改用产品真闸门、沙箱缺席轴按实测登记两轴、P2-32 守卫把备胎轴明示为
       「判不了」；核验脚本改 python-dotenv 装载 .env。
    12. 用例总数 1573 → 1728（上一发布 v20.3.1 → 本发布；`--collect-only` 2026-09-03 实测；
