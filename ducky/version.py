...
v20.3.2 (第 10 轮独立审计整改 · 零凭据首跑 · 2026-09-02)
    ...
    5. 新增判据 is_backend_not_configured：不看异常、不看状态码，看世界的事实
       （配置文件在不在、凭据是不是真的）。四种成因必须分开，其中「配了真凭据但
       后端连不上」必须算失败 —— 拿 503 当未启用会让删除报成功而向量点留着，F-02 复活。
    ...
    8. 用例总数 1573 → 1581。本机 1569 passed + 12 skipped（2026-09-02 实测）；
       基础安装路径 1550 passed + 31 skipped（同日干净 venv 实测）；生产机沙箱与
       全轴齐备两行待本树复测后填数。
