...
| Dimension | Status |
|-----------|--------|
| Total cases | **761** (measured via `pytest --collect-only`) |
| Clean dev machine | 749 passed · **12 skipped** — the skipped ones require the host Hermes source tree, unavailable in a bare checkout |
| Complete environment | **761 all green** (with the Hermes source present, all 12 run and pass; verified on production) |
...
> **Why report both 749 and 761**: the same suite yields different numbers in different environments,
...
>
> ```bash
> pytest tests/ -q -rs | tail -1                                 # no host: 749 passed, 12 skipped
> HERMES_SRC=/path/to/hermes-agent pytest tests/ -q | tail -1    # with host: 761 passed
> HERMES_SRC=none pytest tests/ -q -rs | tail -1                 # host present but forced off: 749 passed, 12 skipped
> ```
>
...
> our own production box is exactly that), the first command above actually prints 761 passed, 0 skipped.
