...
| Dimension | Status |
|-----------|--------|
| Total cases | **1290** (measured via `pytest --collect-only`) |
| Clean dev machine | 1278 passed · **12 skipped** — no host Hermes source, git worktree present (measured) |
| Sandbox on the production box | 1289 passed · **1 skipped** — host Hermes source present, no git worktree (the sandbox is a whitelist copy without `.git`). **This row is axis-derived**: 1290 minus the 1 case gated on the git-worktree axis. The last real sandbox measurement was **859 passed · 1 skipped**, on the v20.0 committed tree, when the total was 860 |
| All axes present | 1290 all green · 0 skipped — **measured on the production box, 2026-08-26** (candidate tree cloned from a bundle, `.git` present, all eleven axes available; the v20.2.1 candidate tree measured 1281 all green the same day) |
...
> **Why report both 1278 and 1289**: the same suite yields different numbers in different environments,
> and quoting only one of them misleads the reader. 1278 is measured here; 1289 is **axis-derived** (1290 minus
...
>
> A dev machine lacks the first → 1278 + 12. The sandbox on the production box lacks the second (whitelist copy, no
> `.git`) → 1289 + 1. **Each is missing one, so neither partial environment produces 1290 all green** — the
...
> pytest tests/ -q -rs | tail -1                                 # no host: 1278 passed, 12 skipped
> HERMES_SRC=/path/to/hermes-agent pytest tests/ -q | tail -1    # with host: 1290 passed
> HERMES_SRC=none pytest tests/ -q -rs | tail -1                 # host present but forced off: 1278 passed, 12 skipped
> ```
>
...
> our own production box is exactly that), the first command above actually prints 1289 passed, 1 skipped
...
> so `tests/test_v20_brand_policy.py` has no baseline to diff against. The `with host: 1290 passed` line in
> the code block above requires *all eleven* axes present at once; that complete-axis result was measured on
> the production box on 2026-08-26 (candidate tree, total 1290, zero skips).
