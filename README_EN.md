...
> **Why report both 1220 and 1231**: the same suite yields different numbers in different environments,
> and quoting only one of them misleads the reader. 1220 is measured here; 1231 is **axis-derived** (1232 minus
...
>
> A dev machine lacks the first → 1220 + 12. The sandbox on the production box lacks the second (whitelist copy, no
> `.git`) → 1231 + 1. **Each is missing one, so neither partial environment produces 1232 all green** — the
...
> pytest tests/ -q -rs | tail -1                                 # no host: 1220 passed, 12 skipped
> HERMES_SRC=/path/to/hermes-agent pytest tests/ -q | tail -1    # with host: 1232 passed
> HERMES_SRC=none pytest tests/ -q -rs | tail -1                 # host present but forced off: 1220 passed, 12 skipped
> ```
>
...
> our own production box is exactly that), the first command above actually prints 1231 passed, 1 skipped
