...
| Dimension | Status |
|-----------|--------|
| Total cases | **693** (measured via `pytest --collect-only`) |
| Clean dev machine | 681 passed · **12 skipped** — the skipped ones require the host Hermes source tree, unavailable in a bare checkout |
| Complete environment | **693 all green** (with the Hermes source present, all 12 run and pass; verified on production) |
...
> **Why report both 681 and 693**: the same suite yields different numbers in different environments,
...
>
> ```bash
> pytest tests/ -q -rs | tail -1                                 # no host: 681 passed, 12 skipped
> HERMES_SRC=/path/to/hermes-agent pytest tests/ -q | tail -1    # with host: 693 passed
> HERMES_SRC=none pytest tests/ -q -rs | tail -1                 # host present but forced off: 681 passed, 12 skipped
> ```
>
