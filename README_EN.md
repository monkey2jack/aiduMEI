...
| Dimension | Status |
|-----------|--------|
| Total cases | **682** (measured via `pytest --collect-only`) |
| Clean dev machine | 670 passed · **12 skipped** — the skipped ones require the host Hermes source tree, unavailable in a bare checkout |
| Complete environment | **682 all green** (with the Hermes source present, all 12 run and pass; verified on production) |
...
> **Why report both 670 and 682**: the same suite yields different numbers in different environments,
...
>
> ```bash
> pytest tests/ -q -rs | tail -1                                 # no host: 670 passed, 12 skipped
> HERMES_SRC=/path/to/hermes-agent pytest tests/ -q | tail -1    # with host: 682 passed
> HERMES_SRC=none pytest tests/ -q -rs | tail -1                 # host present but forced off: 670 passed, 12 skipped
> ```
>
