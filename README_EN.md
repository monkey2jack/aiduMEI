...
pytest tests/ -q -rs | tail -1                                 # no host: 1845 passed, 12 skipped
HERMES_SRC=/path/to/hermes-agent pytest tests/ -q | tail -1    # with host: 1857 passed
HERMES_SRC=none pytest tests/ -q -rs | tail -1                 # forced off: 1845 passed, 12 skipped
...
| Dimension | Status |
|---|---|
| Total cases | **1857** (measured via `pytest --collect-only`, 2026-09-10, v20.5 preview this tree) |
| Clean dev machine | 1845 passed · **12 skipped** — **measured 2026-09-10** (v20.5 preview this tree, Python 3.12; complete extras and model cache, only Hermes source absent) |
...
| Sandbox on the production box | 1846 passed · **11 skipped** — **measured 2026-09-10** (v20.5 preview this tree 48ca0cb, separate sandbox venv on the production box: host source present, no `.env`, without ruff/mcp/fastembed; the 11 are `ruff` ×3, `mcp` ×7 and an empty standby-model cache ×1) |
