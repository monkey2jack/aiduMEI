...
| Dimension | Status |
|-----------|--------|
| Total cases | **771** (measured via `pytest --collect-only`) |
| Clean dev machine | 759 passed · **12 skipped** — the skipped ones require the host Hermes source tree, unavailable in a bare checkout |
| Complete environment | **771 all green** (with the Hermes source present, all 12 run and pass; verified on production) |
...
> **Why report both 759 and 771**: the same suite yields different numbers in different environments,
...
>
> ```bash
> pytest tests/ -q -rs | tail -1                                 # no host: 759 passed, 12 skipped
> HERMES_SRC=/path/to/hermes-agent pytest tests/ -q | tail -1    # with host: 771 passed
> HERMES_SRC=none pytest tests/ -q -rs | tail -1                 # host present but forced off: 759 passed, 12 skipped
> ```
>
...
> our own production box is exactly that), the first command above actually prints 771 passed, 0 skipped.
