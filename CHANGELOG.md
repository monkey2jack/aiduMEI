...
- **由此新增第十二条跳过轴**（`ruff` 已安装，门控 2 条），登记表、两份 README 的轴表、生产机沙箱数字（**1457 通过 · 3 跳过**）全部同步。跳过轴普查也跟着扩了能力：原先按行号归属数用例，跳过语句写在共用辅助函数里时**恒得 0**，报错却指向「位点搬家」—— 看起来像轴废了，其实是数法不认得间接调用。现在做一次调用图不动点传播（`tests/test_v20_skip_axis_census.py`）。
- **README 数字守卫的世界模型也落后了一条轴**：它把「git 轴门控数」直接当成「沙箱跳过数」，第十二条轴一出现，它算出 1459/1、实际 1457/3，**报出来的样子像是文档写错了**。改成显式列出「沙箱缺席的轴」并求和（`tests/test_v19_4_1_audit_fixes.py`），加轴时必须回来登记。**假红灯和假绿灯一样害人。**

用例总数 1442 → 1460。本机 1448 passed + 12 skipped；生产机沙箱 1457 passed + 3 skipped（少 git 工作区与 ruff 两条轴）。
