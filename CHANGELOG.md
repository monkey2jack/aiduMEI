...
- **新增判据 `is_backend_not_configured`：不看异常、不看状态码，看世界的事实**。
  `get_memory()` 把「配置文件不存在」「凭据无效」「服务不可达」「代理坏了」**四种成因全部包成 503** ——
  拿状态码当判据等于宣布「凡是后端出问题，都当它没启用」，于是后端抖动时删除报成功而向量点留着，
  **正是 F-02 的原病被一次「修复」重新引进来**。四种情形必须分开，其中「配了真凭据而连不上」算真故障。
  上一版把判据放在整段的 `except` 上，被既有用例（「后端连不上必须 failed」）当场戳穿。
...
- **用例总数 1573 → 1581**。本机 1569 passed + 12 skipped（2026-09-02 实测）；
  **基础安装路径 1550 passed + 31 skipped**（同日干净 venv 实测，新用户实际形态）；
