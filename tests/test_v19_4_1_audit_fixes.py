...
def test_doc_numbers_are_consistent_across_both_readmes():
    ...
    MEASURED_SANDBOX = (1877, 11)  # 2026-09-10 v20.5.0 正式版本树 6781c7d·生产机独立沙箱实测：宿主源码在场、隔离 HOME、含 .git、无 .env，
    # 独立沙箱 venv（requirements* 装齐，无 ruff/mcp/fastembed）；退出 0。
    # 跳过 11 = ruff×3 + mcp×7 + 备胎模型缓存×1（隔离 HOME 使模型缓存目录为空）。
    # 上一基线 (1846, 11) 是 2026-09-10 的 v20.5.0 preview 48ca0cb 树，换树即失效，不做并列。
