...
def test_doc_numbers_are_consistent_across_both_readmes():
    ...
    MEASURED_SANDBOX = (1967, 26)  # 2026-09-11 v20.5.1 本树 de09794·生产机独立沙箱实测：宿主源码在场、隔离 HOME、含 .git、无 .env，
    # 独立沙箱 venv（requirements* 装齐，无 ruff/mcp/fastembed 等可选轴）；退出 0。
    # 跳过 26 条为沙箱缺席轴合计（宿主轴用例照常跑通——hermes 签名契约已对齐新版宿主）。
    # 上一基线 (1877, 11) 是 2026-09-10 的 v20.5.0 正式版 6781c7d 树，换树即失效，不做并列。
