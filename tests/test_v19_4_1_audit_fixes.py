...
def test_yellow_a_readme_claims_are_consistent():
    ...
    #
    # 2026-08-24 那个数字**真的被跑出来了**（生产实机九轴齐备、0 跳过），于是这条断言
    # 的字面要求本身变成了假话 —— 继续要求 README 写「从未实测」，就是要求它撒谎。
    #
    # ★ 它当时其实还**碰巧是绿的**：正文里逐字引述旧措辞（「在此之前标注为『从未实测』」）
    #   让裸词匹配照样命中。一条靠历史引述才通过的断言，正是这个仓库最忌讳的白护栏 ——
    #   它不再守着任何东西，只是还没红。所以在这里明确废止，职责移交给
    #   tests/test_v20_skip_axis_census.py::test_all_axes_number_is_measured_and_attributed
    #   （那条要求：不许再宣称「推导值，从未实测」，且宣称实测**必须带日期**）。
...
def test_doc_numbers_are_consistent_across_both_readmes():
    ...
    MEASURED_SANDBOX = (1738, 5)  # 2026-09-07 v20.3.4 实测：隔离 HOME、含 .git、无 .env，显式模型/数据集路径；退出 0
    # 2026-09-07 v20.3.4 生产机独立全轴 venv 实测；工具/extras/宿主/模型缓存/公开 LoCoMo 数据集齐备。
    # 这一格从前是「待复测 + 推导值」，推导值恰好等于实测值 —— 但推导对了不等于
    # 测过了；换树必须重测后改这里。
    MEASURED_ALL_AXES = (1743, 0)   # 2026-09-07 实测：隔离 HOME、无 .env、显式模型/数据集路径；退出 0
    ...
    MEASURED_BASIC = (1777, 35)   # 2026-09-08 v20.4.0-alpha 候选树干净 venv 实测（Python 3.12，只装 requirements*；退出 0）。跳过 32→35：P1-6 的 3 条 MCP 鉴权行为用例按 mcp_extra 轴诚实跳过（此前是 ImportError 假红）。
