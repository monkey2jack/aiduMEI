...
_MKDTEMP_BASELINE = 62  # v21.2：+1（test_v21_2_memmy_fusion 沿用 v21 同族 sidecar 测试的 FACTS_DB 重定向写法，需模块级临时目录）          # 实测位点数（只数 tests/test_*.py）；新增请连同这个数字一起改（v21：+4 epistemic/provenance/dossier/sidecar 独立临时库；v21.1：+1 众神殿 pantheon 独立临时库；v22：+1 A3 跨殿空 caller 守卫独立临时库；v22：+1 D4 鉴权负向对照模板独立临时库）
