...
#   （生产就摆着 /hermes/hermes-agent）本文件这 12 条一条都不跳 ——
#   读者**无法把「通过」复现成「跳过」**。此前只做了单向（指路径让跳过跑起来），
#   另一半是缺的。**双向可复现才叫可证伪**：一个你没法让它跳过的「跳过」，
#   和一个你没法让它通过的「通过」，同样不可信。
#
#   🔴v20.0 订正：这段原先写的是「跑出来是 0 skipped」。错的 —— 那是把整份 pytest
#   的跳过总数当成了本文件的跳过数。宿主到位只解掉**这一条轴**；生产部署树上还有
#   git 工作区那条轴在跳（拷贝部署没有 .git），实测 832 passed, 1 skipped。
#   本文件的 12 条确实归零了，全局的 0 skipped 从来不成立。
#
# ★ 为什么 ③ 不许回落：
